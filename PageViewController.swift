...
struct PageViewController: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
        ...
        func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewContorller: UIViewController) -> UIViewController? {
            ...
            guard let index = parent.viewControllers.firstIndex(of: viewContorller) else {
                return nil
            }
            // show the LAST view controller when the user swipes back from the first view controller
            if index == 0 {
        ...
        func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
            // retrieve the index of the currently displayed view coontroller
            guard let index = parent.viewControllers.firstIndex(of: viewController) else {
                return nil
            }
            // show the FIRST view controller when the user swipes further from the last view controller
